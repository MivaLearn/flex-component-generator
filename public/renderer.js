...
function renderTextSubtypeFields(container, textSubtype, propertyPathPrefix, fieldIndex = null) {
    ...
    const baseName = (fieldIndex !== null)
...
function addTextSettingFieldRow(fieldsContainer, propertyPathPrefix) {
    ...
    const allowedSubTypes = Object.keys(BASE_TYPE_FIELDS)
        .filter(t => !['group', 'grouplist', 'list', 'textsettings', 'fragment', 'customlookup', 'image'].includes(t)); // Adjusted exclusion list
    ...
    propertyRow.innerHTML = `
        ...
        <div class="accordion-body">
            <div class="property-field" title="Select the data type for this property (e.g., Text, Image, Group). This determines the available configuration options and how the data is handled.">
                <label>Type:</label>
                <div><select name="${currentPropertyPath}[type]">${Object.keys(TYPE_FIELDS).map((type) => `<option value="${type}">${type}</option>`).join("")}</select></div>
            </div>
  ...
  `;
    ...
    function renderDefaultInputElement(inputType, inputId, inputName, options = {}) {
        ...
        switch (inputType) {
            ...
            case 'color':
                ...
                colorContainer.style.display = 'flex'; colorContainer.style.alignItems = 'center';
                inputElement = document.createElement('input'); inputElement.type = 'color';
                let colorTextInput = document.createElement('input');
                colorTextInput.type = 'text'; colorTextInput.placeholder = '#RRGGBB';
                colorTextInput.style.marginLeft = '5px'; colorTextInput.style.flexGrow = '1';
                ...
                inputElement.addEventListener('input', () => { if (colorTextInput) colorTextInput.value = inputElement.value; });
                colorTextInput.addEventListener('input', () => { if (inputElement && /^#[0-9A-F]{6}$/i.test(colorTextInput.value)) { inputElement.value = colorTextInput.value; } });
                colorContainer.appendChild(inputElement); colorContainer.appendChild(colorTextInput);
    ...
    function renderSingleDefault(container, propertyRowElement, parentNamePrefix) {
        ...
        // --- Handle Standard / Simple Types ---
            ...
            // Create the main container for the default input.
             ...
             if (propertyType === 'image') {
                  ...
                  mainImageInput.type = 'text'; mainImageInput.name = valueInputName;
                  mainImageInput.id = inputId; mainImageInput.placeholder = "Default image path";
                  ...
                  respHeading.style.fontWeight = 'bold'; respHeading.style.gridColumn = '1 / -1'; respHeading.style.marginBottom = '5px';
                  ...
                  if (responsiveImagesList) {
                      responsiveImagesList.querySelectorAll(':scope > .responsive-image-row').forEach(respRow => {
                          ...
                          if (respCodeInput?.value.trim() && respPromptInput) {
                              ...
                              respLabel.htmlFor = respInputId; respLabel.textContent = `${respPrompt} (${respCode}):`; respLabel.style.gridColumn = '1';
                              const respControlDiv = document.createElement('div'); respControlDiv.style.gridColumn = '2';
                              const respInput = document.createElement('input'); respInput.type = 'text'; respInput.id = respInputId;
                  ...
                  if (hasResponsiveFields) {
                      ...
                      noRespMsg.style.fontSize = '0.9em'; noRespMsg.style.color = '#777'; noRespMsg.style.marginLeft = '15px'; noRespMsg.style.display = 'block';
             ...
             if (inputElement) { controlContainer.appendChild(inputElement); }
        ...
        if (enableCheckbox?.checked) {
            ...
            if (settingFields?.length > 0) {
                ...
                tsHeading.style.fontWeight = 'bold'; tsHeading.style.fontStyle = 'italic';
    ...
    const populateDefaultsSection = debounce(() => {
        ...
        defaultsContainer.querySelectorAll('input, select, textarea').forEach(input => {
            // Check for valid name attribute.
             if (input.name) {
                 ...
                 if (input.type === 'checkbox') { value = input.checked; } // Should not happen with current boolean handling (uses select)
                     ...
                     if (input.options.length === 3 && input.options[1].value === 'true' && input.options[2].value === 'false') {
                         value = input.value === 'true' ? true : input.value === 'false' ? false : null; // Convert 'true'/'false' string to boolean
                     } else {
                 ...
                 else { value = input.value; } // Text, textarea, number, etc.
                 ...
                 if ((value !== '' && value !== null && value !== undefined) || typeof value === 'boolean') {
                     ...
                     } else if (value !== null) { // Store actual booleans and non-empty/non-null strings/numbers.
        ...
        if (!hasStandardProperties && !hasAdvancedProperties) {
            ...
            noPropsMsg.style.gridColumn = '1 / -1'; noPropsMsg.style.color = '#777'; noPropsMsg.style.fontSize = '0.9em';
         ...
         Object.keys(existingDefaults).forEach(name => {
             ...
             if (input) {
                 ...
                 if (input.tagName === 'SELECT') {
                     ...
                     if (input.options.length === 3 && input.options[1].value === 'true' && input.options[2].value === 'false') {
                         input.value = storedValue === true ? 'true' : storedValue === false ? 'false' : ''; // Restore 'true', 'false', or '' (for null/None)
                     } else {
                 ...
                 } else if (input.type === 'checkbox') { // Should not be hit with current boolean handling
