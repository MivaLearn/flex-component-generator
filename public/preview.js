 ...
 * * @param {boolean} [isInsideAdvanced=false] - Flag indicating if we are currently processing inside the 'advanced' defaults object. // <-- Added parameter
 */
function cleanAndValidateDefaults(defaultsNode, fullPropertyMap, parentPropCode = null, isInsideAdvanced = false) {
    ...
    for (const key in defaultsNode) {
        ...
        // --- Case 0: Handle the special 'advanced' key at the top level ---
        // Recurse into the 'advanced' object if found at the root level of defaults.
        if (key === 'advanced' && !parentPropCode && !isInsideAdvanced) { // Only at the top level
            if (typeof value === 'object' && value !== null) {
                cleanAndValidateDefaults(value, fullPropertyMap, null, true); // Recurse, setting isInsideAdvanced = true
                // Remove the 'advanced' object if it becomes empty after cleaning.
                if (Object.keys(value).length === 0) {
                    delete defaultsNode[key];
                }
            } else {
                // If 'advanced' key exists but isn't an object, remove it.
                console.warn(`[cleanAndValidateDefaults] Invalid 'advanced' structure in defaults. Expected object, got:`, value);
                delete defaultsNode[key];
            }
            continue; // Move to the next key after handling 'advanced'
        }
        ...
        if (propDef && !propDef.isTextSettingField) {
            ...
            if (propData.textsettings) {
                if (typeof propData.textsettings === 'object' && !Array.isArray(propData.textsettings)) {
                    ...
                    cleanAndValidateDefaults(propData.textsettings, fullPropertyMap, key, isInsideAdvanced);
            ...
            if (propData.children && propDef.type === 'group') {
                 if (typeof propData.children === 'object' && !Array.isArray(propData.children)) {
                    ...
                    cleanAndValidateDefaults(propData.children, fullPropertyMap, key, isInsideAdvanced);
        ...
        // or a *text setting field code*.
            ...
            if (fieldDef?.isTextSettingField) {
             ...
             // 2b. If the key is NOT a text setting field code, assume it's a pseudo-class (like 'hover').
                 ...
                 cleanAndValidateDefaults(textSettingOrPseudoClassData, fullPropertyMap, parentPropCode, isInsideAdvanced);
        ...
        // This means the key is likely a child property code.
            ...
            if (typeof childData === 'object' && childData !== null) {
                ...
                cleanAndValidateDefaults(defaultsNode[key], fullPropertyMap, key, isInsideAdvanced);
        ...
        // --- Case 4: Key doesn't match known property, text setting, or nested structure context, AND is NOT 'advanced' (handled in Case 0) ---
        // This could be leftover data or an unexpected key. Remove it unless we are inside 'advanced'.
        // Keys *inside* 'advanced' are expected to be property codes.
        else if (key !== 'advanced' && !propDef && !parentPropDef && !isInsideAdvanced) {
            console.warn(`[cleanAndValidateDefaults] Removing unknown top-level key '${key}' from defaults.`);
            delete defaultsNode[key];
        }
        // If inside 'advanced' and the key is NOT a property definition, remove it.
        else if (isInsideAdvanced && !propDef && !parentPropDef) {
            console.warn(`[cleanAndValidateDefaults] Removing unknown key '${key}' from advanced defaults.`);
            delete defaultsNode[key];
        }
    }
}
...
function processDefaultsPreview(sourceDefaultsData, componentPropertiesDefinition) {
    ...
    // 3. Clean and validate the entire defaults structure recursively.
    //    The modified cleanAndValidateDefaults will handle the 'advanced' key internally.
    cleanAndValidateDefaults(cleanedDefaults, fullPropertyMap); // Initial call starts recursion

    // 4. Return the final, cleaned defaults object with the desired structure.
...
export function setupPreview() {
    ...
    const updatePreviewInternal = debounce(() => {
        ...
        // Also specifically remove the 'advanced' key within defaults if it's empty after cleaning.
        if (raw.defaults?.advanced && Object.keys(raw.defaults.advanced).length === 0) {
            delete raw.defaults.advanced;
             // If removing 'advanced' makes 'defaults' empty, remove 'defaults' too.
             if (Object.keys(raw.defaults).length === 0) {
                 delete raw.defaults;
             }
        }
